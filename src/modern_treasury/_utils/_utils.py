    ...
    *,
    ...
    try:
        ...
        if not isinstance(obj, bytes) and not isinstance(obj, tuple) and not isinstance(obj, io.IOBase):
            raise RuntimeError(
                f"Expected entry at {flattened_key} to be bytes, an io.IOBase instance or a tuple but received {type(obj)} instead. See https://github.com/Modern-Treasury/modern-treasury-python#file-uploads"
            ) from None
